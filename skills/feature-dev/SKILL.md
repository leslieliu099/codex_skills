...
1. Read applicable `AGENTS.md` files and discover canonical requirements, architecture, README, acceptance, code, tests, configuration, and history closest to the feature.
...
- documentation impact for requirements, architecture, README, `AGENTS.md`, acceptance, and conditional contracts or migration guidance, using `None` with a reason where unaffected;
...
7. Synchronize affected canonical project documents after verified behavior. Update requirements only for user-confirmed decisions, architecture only for material structural changes, README only for changed usage, and acceptance with exact fresh evidence. Never infer `ACCEPTED` from passing checks.

## Completion Ledger

Map each planned outcome and its documentation impact to exact changed files and fresh verification. Do not call the feature complete while a layer, migration, error state, document synchronization, or plan item lacks evidence. If automated testing is impractical, state the alternative observation and remaining risk.
